...
function ProductDetails({ cart, setCart }) {
  ...
  // ✅ GET ALL PRODUCTS (for recommended)
  useEffect(() => {
  ...
  return (
    <div style={{ textAlign: "center", padding: "20px" }}>
      <h2>{product.title}</h2>
      <img src={product.image} alt={product.title} style={{ width: "100%", maxWidth: "350px", margin: "20px auto", display: "block" }} />
      <p>{product.description}</p>
      <p><b>Price:</b> ₹{product.price}</p>

      <button
        onClick={handleAddToCart}
        style={{ padding: "10px 20px", backgroundColor: "#27ae60", color: "white", border: "none", borderRadius: "8px", cursor: "pointer", marginTop: "15px" }}
      >
        Add to Cart
      </button>
